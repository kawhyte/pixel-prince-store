...
npx tsc --noEmit         # Type check the app (tsconfig.json excludes scripts/)
npm run typecheck:scripts # Type check scripts/ + lib/ + config/ + sanity/ (tsconfig.scripts.json)
...
- Ownership (PLAN-52): **Fourthwall owns money and fulfilment** (prices, sizes, variants, checkout). **Studio owns everything a customer reads or looks at** (title, description, tags, category, photos). **The masters folder owns the artwork** (the flat art and the card image). `npm run shop:sync` pulls prices and never touches a photo; `--reset-images` throws Studio's photos away and takes Fourthwall's again, naming what it will replace first. Images are seeded from Fourthwall on the first import only, so a new listing is never blank.
- Room photos live on the artwork (`galleryImages`) and show for every finish. The per-finish main photo is `printOffer.mockup`. A new import writes real copy, tags and a category (`lib/listing-copy.ts`) so a listing arrives finished rather than as blanks.
- Which version opens first: `defaultVersion` on the artwork, else the first offer in the list. Reordering offers in Studio changes it too.
- Versions (PLAN-48): a print can sell several versions of the artwork itself (ivory background, midnight). Each version is its own Fourthwall product named `Title (Version)` / `Title (Version) | Framed`; the import folds them into one artwork with one offer per version and finish. `getVersions(art)` / `resolveVersion(art)` in `lib/commerce.ts`; `getActiveOffer(art, finish, version)`; the page owns version and finish and passes both to `CheckoutButton`. Frame color (Black / Red Oak / White) exists on Fourthwall framed products but is not offered as a choice: the import keeps the Black variants.
