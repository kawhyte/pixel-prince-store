...
- Division of labour: the scripts create listings, prices and images; Kenny then tweaks images and text in Fourthwall and Studio. A plain import run never touches what he changed in Studio (description, tags, category, order of offers). `--remockup` deliberately does: it takes Fourthwall's images again for every offer, mockup and room photos both, and is how photos added under **Photography and design** reach the site.
- Room photos (PLAN-51) live on the offer (`printOffer.gallery`), not the artwork, because they are added to one product in Fourthwall. The page shows the selected offer's photos and falls back to `galleryImages` on the artwork.
