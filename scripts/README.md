...
**Who owns what (PLAN-52):** Fourthwall owns prices, sizes and fulfilment. Studio owns title, copy, tags, category and photos. The masters folder owns the artwork. `npm run shop:sync` pulls prices and leaves photos alone; `npm run shop:prices` reports prices that disagree with `TARGET_PRICES`; `npm run shop:doctor` reports whether a print is finished at all (and shares its price comparison with `shop:prices`, so the two cannot disagree); `npm run shop:art` puts the flat artwork on each offer.
